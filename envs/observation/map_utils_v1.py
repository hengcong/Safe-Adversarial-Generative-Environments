...
from utils.config_helper import deep_merge_dicts
...
class ModuleWorld(object):
    ...
    def _render_walkers(self, surface, list_w, world_to_pixel, from_snapshot=False):
        ...
        for w in list_w:
            ...
            if from_snapshot:
                ...
                if not hasattr(w[0], 'bounding_box') or "controller.ai.walker" in w[0].type_id:
                    continue
    ...
    def _render_waypoints(self, waypoint_surface, waypoint_list, world_to_pixel, world_to_pixel_width):
        ...
        for wp in waypoint_list[1:]:
            wp_location = wp[0].transform.location
            lane_width = world_to_pixel_width(wp[0].lane_width * 0.5)
            x, y = world_to_pixel(wp_location)
...
class BeVWrapper(object):
    ...
    def init(self, client, world, carla_map, player, display, route=None):
        os.environ['SDL_VIDEODRIVER'] = 'dummy'

        self.display = display
        pos_observation = (self.display.get_size()[0] - 84 - 10, 10)
        self.bev_display = np.zeros((84, 84))
        self.display.blit(pygame.surfarray.make_surface(self.bev_display.swapaxes(0, 1)), pos_observation)
        pygame.display.flip()
