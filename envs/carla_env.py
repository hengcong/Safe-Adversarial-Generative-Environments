...
class CarlaEnv(gym.Env):
    ...
    def reset(self):
        ...
        self.episode_data["cbf_dev_sum"] = 0.0
        self.episode_data["cbf_dev_count"] = 0
    ...
    def step(self, actions):
        ...
        for slot_idx, act in actions.items():
            ...
            deviation = float(np.linalg.norm(a_safe - a_agent_des))
            slot_cbf_deviation[slot_idx] = deviation
            self.episode_data["cbf_dev_sum"] = self.episode_data.get("cbf_dev_sum", 0.0) + deviation
            self.episode_data["cbf_dev_count"] = self.episode_data.get("cbf_dev_count", 0) + 1
        ...
        for slot_idx in self.rl_slots:
            ...
            if slot_idx in self.dead_slots:
                ...
                deviation = slot_cbf_deviation.get(slot_idx, 0.0)
                if deviation > 1e-3:
                    base_reward -= 0.03 * min(deviation, 1.0)

                if "reward_components" in self.step_data and len(self.step_data["reward_components"]) > 0:
                    self.step_data["reward_components"][-1]["cbf_dev_agent"] = float(deviation)
        ...
        # ===== CBF mean logging =====
        cbf_dev_sum = self.episode_data.get("cbf_dev_sum", 0.0)
        cbf_dev_count = self.episode_data.get("cbf_dev_count", 0)

        if cbf_dev_count > 0:
            cbf_dev_mean = cbf_dev_sum / cbf_dev_count
        else:
            cbf_dev_mean = 0.0

        # 给所有agent写 mean
        if "reward_components" in self.step_data:
            for d in self.step_data["reward_components"]:
                d["cbf_dev_mean"] = float(cbf_dev_mean)
    ...
    def _compute_reward(self, actor_id):
        ...
        # ======================
        # 1) Speed reward (bias toward target speed, not just "moving")
        # ======================
        speed_limit = max(float(self.get_speed_limit(actor)), 1.0)
        x = v / (speed_limit + 1e-6)

        # slightly softer push term: useful for getting out of zero-speed collapse
        r_push = 1.0 - np.exp(-2.0 * x)

        # stronger target matching term
        v_target = 0.9 * speed_limit
        v_tol = max(1.0, 0.15 * speed_limit)
        r_target = np.exp(-((v - v_target) ** 2) / (2 * v_tol ** 2))

        # emphasize target matching more than mere motion
        low_speed_ratio = v / speed_limit
        r_speed = 0.35 * r_push + 0.65 * r_target
        # if low_speed_ratio < 0.5:
        #     penalty = (0.5 - low_speed_ratio) / 0.5  # 0~1
        #     r_speed -= 0.2 * penalty
        r_speed = max(r_speed, 0)
        # ======================
        ...
        r_lane = -float(np.clip((lateral_norm ** 2) * 0.25, 0.0, 1.0))
    ...
    #     x = v / (speed_limit + 1e-6)
    #
    #     r_push = 1.0 - np.exp(-3.0 * x)
    #
    #     v_target = 0.9 * speed_limit
    #     v_tol = max(1.0, 0.2 * max(1.0, speed_limit))
    #
    #     r_target = np.exp(-((v - v_target) ** 2) / (2 * v_tol ** 2))
    #
    #     r_speed = 0.6 * r_push + 0.4 * r_target
    #
    #     # ======================
    #     # 2) Direction alignment with lane (0..1)
    #     # ======================
